...
function generateFamMessageBody($user, $content, $relation) {
    $receiver_name = $content['first_name']." ".$content['last_name'];
    $family_relation = $relation;
    $sender = $user['first_name']." ".$user['last_name'];
    $approve_url = SMTP_ENDPOINT."?type=family&&from=".$user['id']."&&to=".$content['id']."&&relation=".$family_relation."&&status=approved";
    $delete_url = SMTP_ENDPOINT."?type=family&&from=".$user['id']."&&to=".$content['id']."&&relation=".$family_relation."&&status=delete";
...
function generateFriMessageBody($user, $content) {
    $approve_url = SMTP_ENDPOINT."?type=friend&&from=".$user['id']."&&to=".$content['id']."&&status=approved";
    $delete_url = SMTP_ENDPOINT."?type=friend&&from=".$user['id']."&&to=".$content['id']."&&status=delete";
    $message = "";
...
//          Saving data to family table
            $db = getDbInstance();
            $re = $db->insert('tbl_friend', $data_to_db);
            $stat = sendEmail($to, $body);
            if ($stat) {
