...
$query = 'SELECT tmp.who
		FROM (SELECT fam_gp.by_who AS who
                FROM tbl_fam_groups fam_gp
                WHERE fam_gp.id='.$group['id'].'
                UNION
                SELECT fam_gp_mems.who FROM tbl_fam_groups_members fam_gp_mems
		WHERE fam_gp_mems.group_id='.$group['id'].' AND fam_gp_mems.stat=1) tmp
...
function get_fam_group_note_lists($cat, $note_date, $group_id) {
...
function get_fam_group_note__update_lists($cat, $note_date, $group_id) {
    $user_id = $_SESSION['user_id'];
    $db = getDbInstance();
...
if(isset($_POST) && $_POST) {
    if (isset($_POST['cat_id']) && $_POST['cat_id'] && $_POST['mode'] == 'add') {
        ...
        if ($media_type == 'photo') {
            $target_dir = "./uploads/" . $logged_id . "/fam_group_notes/";
...
//                }
        ...
        $rows = get_fam_group_note_lists('', '', $group['id']);
    }
    elseif (isset($_POST['note_view_date']) && $_POST['view_category']) {
        ...
        $rows = get_fam_group_note_lists($view_cat, $newDate, $group['id']);
    }
    elseif (isset($_POST['note_update_date']) && $_POST['update_category'] && !isset($_POST['mode'])) {
        ...
        $rows = get_fam_group_note__update_lists($update_cat, $newDate, $group['id']);
    }
...
//        echo $_POST['note_id']; exit;
        ...
        if($media_type == 'text' && isset($_POST['note_value'])){
            ...
            $rows = get_fam_group_note__update_lists($update_cat, $update_date, $group['id']);
        }
        //    Update photo
