...
function get_fam_group_note_lists($cat, $note_date) {
    ...
    $query = 'SELECT *
                ...
                (SELECT *
                    ...
                    LEFT JOIN (
                    SELECT
                    tbl_fam_groups.id AS groupId,
                    tbl_fam_groups.group_name AS groupName
                    FROM tbl_fam_groups
...
//        }
    ...
    elseif (isset($_POST['note_view_date']) && $_POST['view_category']) {
        ...
        $rows = get_fam_group_note_lists($view_cat, $newDate);
    }
    elseif (isset($_POST['note_update_date']) && $_POST['update_category']) {
        ...
        $rows = get_fam_group_note_lists($update_cat, $newDate);
    }
} else {
    $rows = get_fam_group_note_lists('', '');
}
